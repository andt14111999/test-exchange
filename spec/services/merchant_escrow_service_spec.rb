...
RSpec.describe MerchantEscrowService, type: :service do
  ...
  describe '#cancel' do
    ...
    it 'handles errors during cancellation' do
      ...
      # Create real accounts to avoid validation issues
      usdt_account = create(:coin_account, :usdt_main, user: merchant)
      fiat_account = create(:fiat_account, user: merchant, currency: 'VND')

      # Create a real escrow and stub the cancel! method to raise an error
      escrow = create(:merchant_escrow,
        user: merchant,
        status: 'active',
        usdt_account: usdt_account,
      ...
      allow(escrow).to receive(:cancel!).and_raise(StandardError, 'Cancellation error')
